...
- 项目负责人已批准仅限本地开发/测试的临时 FFmpeg 例外；外部二进制必须通过环境变量提供，仍不得提交或分发。
...
| 当前状态 | 仅允许仓库外本地开发变量启用；尚未进入构建或发布物 |
...
UNICOMP_FFMPEG_PATH 指向仓库外的本地可执行文件
```

`npm start` 和生产构建不会启用该适配器。该例外不代表 FFmpeg 或任何编码器已经获得正式发布许可。
