...
const errorMessages: Record<VideoEditorIpcErrorCode, string> = {
...
export function VideoEditingPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoading(false);
      if (!listResult.ok) {
        setMessage(errorMessages[listResult.error.code]);
        return;
      }
  ...
  function handleError(
    ...
    setMessage(errorMessages[error.code]);
  }
  ...
  async function openDraft(draftId: string) {
    ...
    try {
      ...
      if (!result.value) {
        setMessage(errorMessages.draft_not_found);
        return;
      }
      acceptDraft(result.value);
