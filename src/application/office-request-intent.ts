...
const tableDeliverablePattern =
  /(?:表格|报表|清单|(?:员工|工资|薪资|销售|数据|统计|经营|客户|库存|财务)表|台账)/;

const createPattern =
  /(?:帮我|请|给我|麻烦)?\s*(?:(?:只|就|先|再|直接|简单(?:地)?|尽量|最好)\s*)*(?:做|生成|制作|创建|写|编写|起草|拟定|整理|输出|导出|出)(?:成|一份|一个)?/;
const explicitNewDocumentPattern =
  /(?:重新|从头)(?:做|生成|制作|创建|写|编写|起草)(?:一份|一个)?|另(?:做|生成|制作|创建|写|编写|起草)(?:一份|一个)|新建(?:一份|一个)?/;
const genericDeliverablePattern =
  /(?:汇报|报告|报表|方案|纪要|复盘|计划|简历|清单|文档|表格|合同|通知|说明书|总结)/;
const revisionPattern =
  /(?:修改|改成|改为|改一下|改简洁|改前|调整|调一下|优化|重排|重新排版|替换|换成|更新|补充|补一|再补|增加|新增|加上|加入|加一|加几|删除|删掉|清空|清除|精简|润色|统一|美化|扩写|扩充|丰富)/;
...
const directRevisionPattern =
  /(?:把|将|请|麻烦|帮我|给)[\s\S]{0,80}(?:修改|改|调整|优化|重排|替换|换成|更新|补充|增加|新增|加|删除|清空|清除|精简|润色|美化|扩写|丰富)/;
