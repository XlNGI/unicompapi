...
### 统一生成历史任务卡审查与实施（2026-09-24 更新；实现与本地验证收口，待负责人验收）

本节是统一生成历史的唯一当前方案，复核并替代此前“作品与状态分别作为顶层历史项”的展示方向。负责人已明确授权实施；本轮范围为任务卡读模型、共享历史与五个调用页、视频原有执行链的接管回执、直接测试；不改持久化 Schema，不新增调度器，不触碰真实数据或服务商。后续不改 Git 历史。
