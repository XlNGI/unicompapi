...
当前状态：阶段 7 已完成联调并正式收口；阶段 8 本地设置技术架构与任务拆分已形成，等待项目负责人批准，B1/A1 尚未启动。阶段 8 固定覆盖 10 个设置分类，采用本机版本化设置、能力驱动展示、普通偏好自动保存和高风险操作两阶段确认。生产更新源、生产媒体组件分发、硬件能力与 macOS 实机证据仍未具备，不得伪造为可用。规划见 `docs/active/阶段8-技术架构.md` 与 `docs/active/阶段8-任务拆分.md`。
...
当前未完成项：规划等待项目负责人批准，B1/A1 未启动；生产更新源与签名安装链路、生产媒体组件分发、硬件能力和 macOS 实机证据仍未具备。不得把这些能力伪装为可用。
