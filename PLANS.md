...
### 统一生成历史任务卡审查与总方案（2026-09-23；方案完成，待授权实施）

本节是统一生成历史的唯一当前方案，复核并替代此前“作品与状态分别作为顶层历史项”的展示方向。当前请求授权的是项目审查和方案写入，不包含产品代码、Git 历史、真实数据或服务商操作。

审查基线：`D:\unicompapi-worktrees\unified-generation-history`，分支 `codex/unified-generation-history`，HEAD `4af46b5fe49710468587ae7e3caa98490d135bdd`；审查开始时工作树干净，`origin/develop` 是该分支祖先。分支相对 `origin/develop` 的文件差异含基础剪辑/视频导出等无关变更；后续若实施并准备 PR，必须先将本功能改动隔离到只含生成历史的干净分支。未修改代码或 `D:\unicompapi-git`，未读取真实项目任务/作品数据，未调用真实服务商。
...
本方案是 D2、T2 的跨读模型/IPC/UI 任务身份契约变更；布局通过可复现 Electron 门禁 T3 验证。当前阶段仅完成只读审查和本计划写入，产品代码实施须负责人明确授权。
...
- 本轮完成直接代码审查、两张附件图核对及总方案写入；未改源码或执行测试。
- 当前分支已有快速生图按持久化 workspace mode 查历史、任务/作品身份改进、状态选择和 CSS 布局改动；定向 Node/UI 静态契约曾通过 49/49。`outputs/generation-history-audit/report.json` 及合成 IPC 的隐藏 Electron 探针可作为隔离诊断材料，不是完整 App/Electron 端到端验收，也没有截图所示任务的真实 ID 对照。
- 未完成：taskId 分组读模型和单任务卡；任务组活动项与结束项分页；存储事件合并刷新回归；草稿删除后的真实读模型回归；完整 AppLayout/main/preload/IPC 验收；当前 CSS 完整页面对照；视频长时间 busy 的后台接管审计；实际项目人工验收。
- 不调用已耗尽预算的 Vidu，不读取凭证或真实用户数据。
- 下一步：负责人确认是否按本方案授权实施。方案中的任务卡聚合只解决“一任务多卡”问题；按钮长时间“处理中”和现有布局是否仍跳动需各自通过其专门验收，不能据此宣称一并解决。
