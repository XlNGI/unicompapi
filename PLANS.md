...
2026-08-28 工程补充：按负责人截图与交互决策完成任务中心消费概览单蒙版与滚动折叠，并根据连续验收反馈关闭触发缺陷。人民币消费柱状图和供应商人民币消费占比共享一个主题面板与响应式分隔线；任务中心页面任意位置累计向下滚轮/触控板滑动 16px 收起概览，累计向上 12px 恢复。此前“抽搐”首先来自页面滚轮与内部任务栏 `scrollTop` 两套状态控制竞争，现已删除内部状态写入，只保留根节点唯一控制路径；右侧详情内容不足以滚动时还会产生触控板边界反弹，现进一步把相邻间隔不足 180ms 的输入锁定为同一手势，同一手势内的反向增量不能展开，必须停止至少 180ms 后开始明确反向手势。每次状态切换另有 420ms 方向锁覆盖 320ms 动画。展开容器不再被 Flex 挤压，1280×720 下七日数据完整显示。定向 UI 合同 10/10、全量 Node/UI 328/328、Vitest 888/888，共 1216/1216 通过；TypeScript、定向 ESLint、生产构建和差异检查通过。本地浏览器连续滚轮反弹序列后稳定隐藏，明确新向上手势才恢复，控制台 0 warning/0 error。未调用真实服务商、未读取凭证、未产生费用；阶段 10 与 macOS 延期边界不变。记录见 `docs/active/任务中心消费概览蒙版与滚动折叠-2026-08-28.md`。

2026-08-28 工程补充：完成任务中心人民币消费摘要读模型优化。新增主进程受控 `getConsumptionSummary`、共享 DTO、Electron IPC/preload 白名单与批准换算事实端口；全局消费图表从 renderer 最多 201 次列表/详情 IPC 收敛为一次摘要读取，并随本地存储变化刷新。金额在主进程使用精确十进制有理数聚合，UI 只展示人民币；没有批准换算事实的 USD 等费用明确保持“待换算”，不混入人民币总额，也不把原数值直接改标“元”。供应商按稳定 ID 聚合，前 5 个外归入“其他”，占比基点严格合计 100%，日期桶连续补零；DTO 不返回换算率、价格规则或受保护路由/凭证事实。全量 Node/UI 327/327、Vitest 881/881，共 1208/1208 通过；TypeScript、ESLint、生产构建、交接校验、平台审计、恢复审计、阶段 9 关闭校验、Electron 运行时集成、安全存储与差异检查通过。Windows 生产 Electron 隔离可见启动新增 4 个进程、4/4 响应、1 个 `UniComp` 窗口，stderr 为空，优雅关闭后残留 0；UI Automation 未暴露导航语义，真实项目图表逐项视觉核对未伪记为通过。未调用真实服务商、未读取真实凭证、未产生费用；生产组合尚未注入批准汇率，macOS 继续 deferred，阶段 10 未启动。记录见 `docs/active/任务中心人民币消费摘要读模型-2026-08-28.md`。

2026-08-27 失败复核：负责人在 Vidu 精确映射上线后再次提交 `viduq3-turbo` 图生视频。本地不可变路由快照确认使用 `parameters.unicompapi.viduq3_turbo.image_to_video.official_mapping` revision 2；创建请求 `POST /v1/videos` 返回 HTTP 200，后续查询均返回 HTTP 200，但远端任务约 18 秒后进入 `failed`。同一模型在精确映射前已有三次通用 Schema 请求呈现完全相同的“创建成功、异步失败”，另有一次文生视频也异步失败，因此当前失败不是旧 Profile、Schema 未迁移、创建端点或单一图片封装造成，客户端不得继续猜改字段。现补齐远端失败终态的安全原因解析：余额/额度、模型不可用、内容安全、图片不可用映射为固定脱敏文案，未知原因仅保留受限机器码，不落盘上游原始正文、Prompt 或凭证。历史响应未保存原始失败对象，无法追溯本次具体上游码；需由 UniCompAPI 后台按其任务/request ID 核对供应商失败原因。全量 Node 323/323、Vitest 875/875、TypeScript、定向 ESLint 与 `git diff --check` 通过；复核过程未发起新请求、未读取凭证、未产生额外费用。记录见 `docs/active/UniCompAPI-Vidu官方参数映射补齐记录-2026-08-27.md`。

2026-08-27 工程补充：补齐 UniCompAPI 下 Vidu 的官方参数合同映射。仅对已有实例证据的精确组合启用：`viduq3-turbo` 保留文生视频与图生视频，`viduq3-pro` 保留文生视频；三条能力均复用官方 Vidu revision 2 的 `audio`、`duration`、`resolution`、`aspect_ratio`、`seed` 字段语义与枚举/范围，并注册为使用 NewAPI 结果、用量和约束合同的 UniCompAPI 专用 Schema。旧通用视频 Profile 会自动迁移到精确 Schema。提交仍走 UniCompAPI `POST /v1/videos`；官方参数位于顶层，图生视频继续使用已有成功历史证据的单张顶层 `image` data-URL，不切换为未经该网关验证的 `images[]`；`audio` 缺省与官方 q3 合同一致发送 `true`。当前实例已明确返回 `unsupported_model` 的 `viduq3`、`viduq3-mix` 保留为已知目录键但不再提供可提交视频候选；其他 OpenAI-compatible 包的同名模型继续使用通用网关合同。定向 Vitest 93/93、全量 Node 323/323 与 Vitest 873/873、TypeScript、定向 ESLint 和 `git diff --check` 通过；未调用真实服务商、未读取凭证、未产生收费请求。阶段 10 与 macOS 延期边界不变。记录见 `docs/active/UniCompAPI-Vidu官方参数映射补齐记录-2026-08-27.md`。

2026-08-27 后续修正：UniCompAPI Seedance 图生视频当前发送已验证的顶层 `model` + `prompt` + 单张 `image` data-URL；此前 `content[]` 结构在实例前置层被拒绝且后台无调用记录，因此不再发送。`network.log` 同时记录脱敏的序列化请求字节数，便于与边缘 `Content-Length` 核对。

2026-08-27 二次修正：根据 UniCompAPI 官网 Seedance 页面实际前端代码，精确 Seedance 图生视频请求恢复为 `model` + `metadata` + `content[]`；图片位于 `content[].image_url.url`，控制参数位于 `metadata`。官网使用 MinIO 公网 URL，桌面端当前仍使用受控本地图片 data-URL，需后续取得实例媒体上传合同后再替换。

2026-08-27 工程补充：完成 UniCompAPI Seedance 2.0 图生视频请求体与结果状态收敛。精确 `doubao-seedance-2-0-260128` / `doubao-seedance-2-0-fast-260128` 的图生视频现在使用官方 `content[]` 多模态首帧结构（`text` + `image_url` + `role=first_frame`），普通 NewAPI 路由继续保留兼容的 `prompt`/`image` 结构；新增请求发出后的明确服务商拒绝状态 `failed`，网络、超时、代理和损坏响应仍为 `unknown_outcome`，并将安全 HTTP 状态、上游 `code/type` 与受控 request ID 写入本地 `network.log`，不写入 Token、URL、Prompt、Base64 或原始错误正文。重启 Electron 后按受控项目草稿的本地事实记录了两次真实提交尝试（`2026-08-27T04:22:56Z` 与 `2026-08-27T04:55:28Z`）：第一次任务 `task-video-d5bce72a-920c-400e-b07f-662dce851ed8` / 调用 `attempt-7bce577d-7efe-4d0b-9827-2b1761c2aeeb`，第二次任务 `task-video-8aae986e-e491-403b-a64d-dba11f2f7c0a` / 调用 `attempt-a5833996-a7c1-40ce-889b-8b915dccaa59`；两次 POST `/v1/videos` 均收到 HTTP 400，安全上游码 `invalid_tokenpony_request`，映射为 `newapi.invalid_request`，均落盘为 `failed`（阶段 `submitting`、不可重试），无远端 operation ID、轮询、下载、Usage、正式 Work。该结果只证明客户端收到固定地址前置层返回的机器可识别拒绝；负责人反馈 UniCompAPI 后台没有对应调用记录，因此不能证明请求进入业务网关或模型通道。取得实例级 TokenPony 视频合同、确认请求记录链路前，不得再次收费尝试或猜测字段。记录见 `docs/active/UniCompAPI-Seedance图生视频真实验收与拒绝记录-2026-08-27.md`。

2026-08-26 工程补充：修复 UniCompAPI Seedance 2.0 精确 Schema 迁移后模型无法出现在视频选择器的问题。根因是 `doubao-seedance-2-0-260128` 与 `doubao-seedance-2-0-fast-260128` 已被视频 Profile 绑定到 Seedance 官方文生/图生 Schema，但候选源和 NewAPI 视频适配器共享的合同注册表只登记通用 NewAPI Schema，合同解析失败后候选被跳过。现将两个精确 Schema 以既有 NewAPI 视频结果、用量与约束合同登记至共享集合；模型列出、选择后的参数展示和提交前 Schema 解析一致。新增精确合同解析回归。验证：定向 71 项、全量 `pnpm test`、TypeScript、ESLint、生产构建和差异检查通过；未调用真实服务商、未读取凭证、未产生收费请求。记录见 `docs/active/Seedance视频候选合同修复记录.md`。

2026-08-25 工程补充：为大学课堂 PPT 正式生成链路增加整份文档级组件规划。结构化强语义章节继续锁定图表、表格、图文、时间轴、对比、指标或提醒组件；普通要点章节结合上一页与全局使用次数在卡片和正文列表间确定性轮换，避免连续重复，同时不使用随机选择。原始固定母版准入与程序化渲染共用同一规划结果，防止母版绕过防重复策略。组件/PPT 定向回归 20/20、UI 合同 5/5、TypeScript、定向 ESLint、生产构建和 `git diff --check` 通过；未调用真实 Provider 或收费服务。后续可继续增加同一组件的多种视觉变体，降低跨页重复感。

2026-08-25 工程补充：修复对话内文档生成的 `invalid_outline` 易失败问题。解析入口现在可从带前后说明的 JSON 中提取对象，兼容模型省略 `kind`、将 `level` 返回为字符串等轻微偏差，同时继续拒绝未知 block、错误表格和非法 JSON；聊天页错误提示保留具体字段原因。解析回归 18/18、组件/PPT 回归 16/16、UI 合同 5/5、TypeScript、定向 ESLint、生产构建和 `git diff --check` 通过。

2026-08-25 工程补充：补充 9 章节大学课堂 PPT 回归场景，验证中间章节命中“结果评估”时会放弃固定三卡片母版并切换指标组件，生成 11 页且保留章节内容；定向 PPT 回归 16/16、生产构建通过。旧 `tmp/*.pptx` 文件仍为历史样例，不能作为当前生成链路验收依据。

2026-08-25 工程补充：继续推进 PPT 组件化：大学课堂主题新增语义对比组件（双栏），包含“传统/现在、优点/缺点、对比/区别”等两项内容时由选择器自动选用；原始母版仅对明确匹配卡片能力的章节启用，时间轴、对比、图表、表格和图文内容不再被固定母版强制压入卡片槽。定向 Vitest 19/19、TypeScript、定向 ESLint、生产构建和 `git diff --check` 通过。仍待后续接入 LLM 组件意图 JSON、稳定槽位 ID和更多组件变体。

2026-08-24 工程补充：按负责人确认的组件化方案，为 PPT 生成新增与主题解耦的版本化组件注册表（标题、正文、卡片、时间轴、对比、表格、图表、图文、结束页），系统依据大纲语义在受约束候选中选择组件；大学主题程序化路径接入时间轴变体，原始大学母版增加容量检查、超限回退和动态文本槽位清空，避免内容丢失或旧示例残留。新增组件注册与选择测试；定向 Vitest 18/18、UI 合同 5/5、TypeScript、定向 ESLint、生产构建和 `git diff --check` 通过。未调用真实 Provider、未读取凭证、未产生收费请求。组件仍需后续补齐稳定槽位 ID、LLM 组件意图 JSON 和 PowerPoint/WPS 人工视觉验收。

2026-08-26 工程补充：完成 UniCompAPI Seedance 2.0 的精确官方请求体映射。仅 `doubao-seedance-2-0-260128` 与 `doubao-seedance-2-0-fast-260128` 在 UniCompAPI 包下绑定官方字段 Schema，并将 `resolution`、`ratio`、`duration`、`frames`、`seed`、`camera_fixed`、`watermark`、`generate_audio`、`return_last_frame` 投影至 JSON 顶层；普通 OpenAI-compatible 中转站即使同名也保持通用网关 Schema 和 `metadata` 合同，不误发送官方扩展字段。旧 Seedance Profile 自动迁移到精确 Schema，`duration` 与 `frames` 同时填写会在 HTTP 前拒绝。Fast 模型公开资料未确认必填或枚举值，UI 不猜测。验证：定向 66/66、全量 `pnpm test`、TypeScript、ESLint、生产构建和差异检查通过；未调用真实服务商、未读取凭证、未产生收费请求。记录见 `docs/active/Seedance请求体字段对齐-验收记录.md`。

2026-08-26 工程补充：统一图片与视频创作页的动态参数标准。候选 Schema 中 `required: true` 或 `exposure: user_required` 的字段现在必须显示并标记“必填”；前端会在输入阶段校验空值、枚举、数值/整数范围与步长、数组、JSON 对象和布尔值，中文字段错误会禁用生成按钮，`prepare()` 再次短路，错误不会进入草稿保存、主进程准备或提交。快速页仅在当前候选没有动态字段时隐藏表单，不再从枚举首项猜测必填值。Schema 核对确认 Seedream 5 与 Qwen Image 的 `size` 已为精确必填；共享 OpenAI 兼容视频 Schema 不将仅对部分 Sora 格式有证据的 `seconds` 错设为全局必填，等待精确模型合同与路由映射。验证：NewAPI 50 项、domain/platform 16 项、UI 49 项、全量测试、TypeScript、ESLint、生产构建通过；未调用真实服务商、未读取凭证、未产生收费请求。记录见 `docs/active/动态参数前端校验与必填适配-验收记录.md`。

2026-08-26 工程补充：按负责人截图反馈移除专业生图、文生视频和图生视频提交栏中的“可以提交”字样，保存完成后统一只显示“当前配置已保存”，避免把本地持久化事实误表述为模型、参数和运行门禁均已满足。新增 UI 合同禁止三页恢复该措辞。验证：定向 UI 合同通过，TypeScript 与差异检查通过；未改变生成、候选或提交逻辑。

2026-08-26 工程补充：在专业生图同类修复后补齐图生视频单图源状态收敛。图生视频页面已经把 `selectMaterial/importMaterial/clearMaterial` 结果作为主进程已持久化事实处理，但 `image_source` 后端仍返回 `editing`，导致页面不会再排入自动保存、模型候选却持续被保存状态门禁阻塞。现仅将图生视频 `image_source` 的添加、拖入和清除结果收敛为 `saved`；快速视频和文生视频动态素材槽语义不变，单图约束、校验、预览、修订与提交门禁不变。验证：定向平台 7 项与 UI 11 项通过；全量 Node/UI 317 项与 Vitest 840 项，共 1157 项通过，0 失败、0 跳过；TypeScript（app/test）、目标 ESLint、生产构建和 `git diff --check` 通过。未调用真实服务商、未读取凭证、未产生收费请求。记录见 `docs/active/图生视频添加图片后模型选择恢复-修复记录.md`。

2026-08-26 工程补充：修复专业生图添加参考图片后模型选择器长期停留在“正在保存当前配置”的状态收敛问题。根因是受控选图、拖入图片、引用本地作品和清除图片已经在主进程串行持久化完整草稿，却仍返回 `editing` 并由 renderer 当作未保存修改再次排入自动保存，模型候选因此被冗余保存门禁阻塞。专业生图本地媒体变更现直接返回 `saved`，renderer 使用已持久化回调更新草稿；图片校验、项目内登记、修订冲突和提交前模型候选校验保持不变。验证：Node/UI 317 项与 Vitest 840 项，共 1157 项通过，0 失败、0 跳过；TypeScript（app/test）、目标 ESLint、生产构建和 `git diff --check` 通过。未调用真实服务商、未读取凭证、未产生收费请求；阶段 10 与 macOS 延期边界不变。记录见 `docs/active/专业生图添加图片后模型选择恢复-修复记录.md`。

2026-08-25 工程补充：按模型适配审查与官方参数核对在 `feature/model-adaptation-hardening` 实施加固（未合并）。① UniCompAPI 改为封闭世界路由：未知模型 key（目录同步新增或手工注册但未登记能力）不再自动获得 text_chat/text_reasoning/text_to_image/text_to_video/image_to_video Profile 与候选，必须显式补能力表、Schema、路由测试后启用，消除与“全模型适配方案”验收标准“不存在未声明能力的模型 Profile / 暂不接模型不进入候选”的偏差。② 核对 UniCompAPI 官方 Apifox 合同（ChatCompletions/生成图像 OpenAI 与通义千问格式/创建视频 Sora 格式）后新增 UniCompAPI 专用文本 Schema：只开放官方字段（chat：max_tokens/temperature/top_p/stop/n/presence_penalty/frequency_penalty/seed/response_format/tool_choice/user/logit_bias；reasoning：max_completion_tokens/reasoning_effort/stop/n/presence_penalty/frequency_penalty/seed/response_format/tool_choice/user/logit_bias），不再向 UniCompAPI 暴露或发送官方合同外的 thinking/chat_template_kwargs/enable_thinking/top_k/metadata（DeepSeek 事故已证明网关拒收未公开字段）；DeepSeek V4 reasoning_effort 特判收敛到能力表。③ 常量替换 `'provider-package-unicompapi'` 字面量、删除未使用的 isUniCompApiViduModel。④ 测试同步：未知模型零 Profile、UniCompAPI 文本请求只序列化官方字段且非官方字段在 HTTP 前被拒。验证：domain+platform Vitest 816+2 项全部通过（142+1 文件）、tsc（app/test）、目标 ESLint、生产构建、git diff --check 通过；未调用真实服务商、未读取真实凭证、未产生收费请求。记录见 `docs/active/模型适配加固与官方参数核对-验收记录.md`。
...
2026-08-24 工程补充：对话内 PPT 生成发现图表可因同页前置正文/表格累计纵坐标越过 16:9 页面底部，且带图片时仍使用满宽图表造成重叠。PPT 生成器现将每个原生图表放入独立图表页，并在有图片的纯图表页使用左右分栏安全尺寸；新增 PPTX XML 回归测试，验证带图片、7 个分类标签的图表右下角不超过 `13.333 x 7.5` 英寸页面边界。`tests/platform/office-document-generator.test.ts` 11/11、TypeScript、定向 ESLint、生产构建与 `git diff --check` 通过。未调用真实 Provider、未读取凭证、未提交或推送；仍需 Windows PowerPoint/WPS 人工打开新版文件确认视觉效果。

2026-08-24 工程补充：`feature/ppt-university-classroom-template` 将用户提供的《人工智能入门_大学生汇报.pptx》的可编辑视觉语言提炼为对话文档模式内置“大学课堂汇报”主题：青绿主色与蓝/橙/红辅助色、课堂封面、章节页、四色知识卡、表格页和独立图表页均由原生 PPT 元素生成。超过四项要点、多段说明或超过六行表格时自动分页，避免内容静默丢失；图表继续采用已验证的 16:9 边界安全布局。主题/生成器测试 16/16、聊天 UI 合同 5/5、TypeScript、定向 ESLint、生产构建与 `git diff --check` 通过。未调用真实 Provider、未读取凭证、未提交或推送；Windows PowerPoint/WPS 人工视觉验收仍待完成。工程记录见 `docs/active/对话内Office文档生成-大学课堂汇报模板验收记录.md`。

2026-08-24 工程补充：新增 `scripts/generate-reference-template-test.mjs`，用于只读加载用户提供的原始 PPTX、保留页面图形与布局并替换指定文本，生成 `tmp/reference-template-test.pptx` 测试样例。样例共 10 页，第 3 页形状数量保持 `26 -> 26`，时间轴圆点、横线、间距和样式来自原始母版；原始桌面文件未被覆盖。该脚本当前用于视觉试测，尚未接入对话页正式生成链路。
2026-08-26 Engineering record: `feature/seedance-2-parameter-contract-ui` separates the UniCompAPI Seedance 2.0 standard and Fast model parameter schemas. UI enum controls now use documented resolution, ratio, and duration values. Actual required inputs (model, prompt, and image-to-video first frame) are marked and locally checked before submission; optional provider controls are not incorrectly marked required. The verified UniCompAPI `/v1/videos` gateway request shape remains unchanged, while unsupported Seedance values are rejected before HTTP. Evidence and acceptance notes: `docs/active/2026-08-26-seedance-video-parameter-contract-record.md`. No provider HTTP call, credential read, or paid generation was performed. Full test, typecheck, lint, build, and diff validation are pending at this record point.

## 工程性能优化专项｜生成历史与任务中心（2026-08-28 规划）

2026-08-28 只读诊断确认生成历史、全局任务摘要和任务中心调用时间线存在多层全量扫描与 N+1：当前匿名规模为 8 个可用项目、286 个任务、153 个作品和 289 次调用；一次 `listTasks()` 估算重复读取约 12.58 MiB，生成历史在详情、句柄和媒体加载前的基础扫描约 28.73 MiB；任务时间线可触发 1 次调用列表加最多 200 次逐条详情 IPC。方案按 PR0—PR6 依次建立性能基线、项目批量快照、单次任务时间线与前置分页、共享任务 Store 与消费汇总缓存、生成历史分页与按需媒体，并仅在前述优化仍不达标时实施可重建磁盘索引/缩略图缓存。本专项涉及的任务、调用、用量观察、本地结果与作品继续以对应项目事实文件为权威来源，索引和缓存均可删除重建；不引入数据库，不弱化文件校验和作品登记，不调用真实服务商、不读取凭证、不产生费用，也不启动阶段 10。当前仅形成方案，尚未授权或启动实现；计划文档：`docs/active/生成历史与任务中心性能优化方案-2026-08-28.md`。
...
2026-08-28 图片结果接收优化 PR3 与 Windows 收口：`feature/provider-neutral-image-result-receipt` 已用 `StoredImmediateImageResultPort` 替换生产接收链路中的 Vidu 专属命名，按持久化 Provider Operation 归属读取唯一 `base64/remote_url/file_uri` 图片结果；远端地址在下载前限制为无凭证、无 fragment、非 IP/localhost/私网语义的 HTTPS，并把禁止私网、DNS 重绑定保护、禁止重定向和禁止携带凭证固化为受控下载契约。接收器新增从开始、描述读取、下载、校验到 Work 登记/失败的白名单安全事件，Electron 只把 ID、安全阶段/错误码/retryability/时间戳写入 `logs/image-result-receipt.log`，日志失败不影响主流程，URL、签名查询、base64、Token、正文和原始异常不进入日志。定向平台测试 20/20；全量 Node/UI 327 项与 Vitest 888 项，共 1,215 项通过、0 失败，另有 5 个既有媒体工具链/真实导出用例如实跳过；TypeScript、ESLint、生产构建、交接校验、平台审计、恢复审计、阶段 9 收口、运行时集成、安全存储与差异检查通过。Windows 生产 Electron 4/4 响应、1 个可见窗口、stderr 0、正常关闭后残留 0。真实服务商 HTTP/DNS、凭证读取/验证、收费调用与费用均为 0；macOS 保持 `not_run/deferred`，阶段 10 未启动。记录见 `docs/active/图片结果接收优化-PR3-服务商无关接收与Windows收口记录.md`。

2026-08-28 当前项目任务状态栏范围修复：`feature/current-project-task-status` 将共享任务快照与当前项目 Session 同批读取，底部生产任务展开面板、折叠状态计数及左侧“最近任务活动”统一按当前 `projectId` 投影；任务中心继续保留全局任务列表和项目筛选。项目切换事件会立即失效旧项目范围，并在并发刷新期间登记 latest refresh，避免旧项目任务闪现或变更通知丢失；未打开项目时不回退到全局任务。新增混合项目、无项目和项目切换回归测试。全量 Node/UI 327 项与 Vitest 891 项，共 1,218 项通过、0 失败，另有 5 个既有媒体工具链/真实导出用例如实跳过；两套 TypeScript、ESLint、生产构建、性能门禁 4/4、交接校验、平台审计、恢复审计、阶段 9 收口、运行时集成、安全存储与差异检查通过。Windows 生产 Electron 4/4 响应、1 个可见窗口、stderr 0、优雅关闭后残留 0。未调用真实服务商、未读取凭证、未产生费用；macOS 保持 `not_run/deferred`，阶段 10 未启动。记录见 `docs/active/当前项目任务状态栏范围修复记录.md`。
