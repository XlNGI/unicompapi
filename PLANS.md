...
当前状态：阶段 7｜B1、B2、A1、A2 与 A3 已完成并合并，A3 已通过项目负责人 Windows Electron 手工验收。项目负责人已批准 Apache-2.0 项目许可证、FFmpeg 8.1.2 LGPL-only 选型方向，以及仅限本地开发/测试的项目 `.tools/` 工具链规则。Windows FFmpeg `n8.1.2` 已通过固定 SHA-256、版本、LGPL-only 构建参数、编码器、许可证、代理、缩略图、波形和缓存复用验证；`.tools/` 与二进制不得提交或进入发布物。B3 正式二进制分发仍等待编解码器白名单、专利/商业分发审查和跨平台构建记录。
