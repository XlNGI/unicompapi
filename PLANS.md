...
当前等待 B4 非快进合并 `develop`。合并后按顺序启动 A4；Windows 原生目录授权/撤销、真实睡眠/锁屏、可见通知/声音/系统设置仍是 A4 最终实机项。macOS 保持 `not_run/deferred` 的非必需目标，适配与未来验收入口继续保留。
