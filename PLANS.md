...
2026-08-20 工程补充：`feature/dynamic-parameter-ui` 优化创作页动态参数表单。参数按轻量双列网格组织，补充“已设置/使用默认值”状态、数值范围与步长、数组分隔和 JSON 格式提示；布尔参数改为带“开启/关闭”文本的开关，并补齐控件可访问名称与错误态。修复图片与视频动态参数编辑期间因自动保存清空候选、卸载表单造成的跳动和焦点丢失：未保存时仅暂停候选刷新并保留当前参数合同。专业生图把“项目上下文”和“提示词增强”收敛为紧凑操作行；增强入口显式展开动态模型选择器，只有“确认增强”触发增强提交；模型下方重复的参数合同、费用和可准备信息块不再展示，底层提交校验保持不变。参数字段 ID、原始值、默认值与提交合同保持不变；完整门禁结果见验收记录。未调用真实服务商、未读取凭证或产生收费请求，阶段 10 未启动。验收记录见 `docs/active/动态参数UI优化验收记录.md`。
