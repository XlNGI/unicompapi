...
当前状态：阶段 7 已完成联调并正式收口；阶段 8 规划已批准，B1 已合并 `develop@1eb3dd8`，B2 已在 `feature/local-settings-storage-performance-media` 完成并等待合并。B2 建立受控目录登记、路径隔离 DTO、目录迁移与故障恢复、清理白名单、动态性能策略快照、真实 FFmpeg 状态和两阶段高风险执行，完整门禁为 346 项测试通过、0 失败、0 跳过。生产更新源、生产媒体组件分发、硬件能力与 macOS 实机证据仍未具备，不得伪造为可用。规划见 `docs/active/阶段8-技术架构.md` 与 `docs/active/阶段8-任务拆分.md`，B1/B2 记录见 `docs/active/阶段8-B1-设置契约与仓储记录.md`、`docs/active/阶段8-B2-存储性能与媒体能力记录.md`。
...
下一步：合并 B1 后，开发者 A 可启动 A1 设置页壳与常规面板，开发者 B 可启动 B2 存储、任务性能和本地媒体端口。工程记录见 `docs/active/阶段8-B1-设置契约与仓储记录.md`。
...
受控 IPC 新增系统状态和原生目录选择，目录迁移、清理、性能/并发、高性能和硬件偏好复用 B1 的一次性 plan/confirm/execute 句柄、过期与 revision 复检。完整门禁为 117 项 Node UI/IPC/工具链测试与 229 项 Vitest 领域/平台测试，共 346 项通过、0 失败、0 跳过；TypeScript、ESLint、生产构建和差异检查通过。Windows Electron 生产构建隐藏启动新增 4 个进程，全部保持响应，结束后无本次残留进程。

未完成项：A2 的存储/性能/媒体面板尚未接入；B3/B4 的隐私、代理、通知、快捷键、诊断和更新不在本分支；硬件编码、生产 FFmpeg 分发、签名、编解码器白名单、专利/商业审查和 macOS 实机证据继续等待阶段 9/10。B2 合并 `develop` 后，A2 才可从同时包含 A1 与 B2 的最新基线启动。工程记录见 `docs/active/阶段8-B2-存储性能与媒体能力记录.md`。
