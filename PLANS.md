...
当前处于：阶段 2｜本地领域基础，领域与 UI 契约已合并，正在补齐真实本地存储、恢复服务与受控存储 IPC。
...
- 阶段 2 最小受控存储 IPC 已建立：渲染进程仅能按 fileId 调用探测、校验、原生文件选择 relink 和索引重建；主进程持有项目根目录，DTO 不返回绝对路径或原始 Hash。活动项目 session 尚待项目打开流程接入。
