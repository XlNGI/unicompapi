...
### 统一生成历史任务卡审查与实施（2026-09-24 更新；实现完成，合并后全应用验收仍有环境阻断，待负责人验收）

本节是统一生成历史的唯一当前方案，复核并替代此前“作品与状态分别作为顶层历史项”的展示方向。负责人已明确授权实施；本轮范围为任务卡读模型、共享历史与五个调用页、视频原有执行链的接管回执、直接测试；不改持久化 Schema，不新增调度器，不触碰真实数据或服务商。分支同步 develop 使用普通合并，保留已有提交历史。

审查基线：`D:\unicompapi-worktrees\unified-generation-history`，分支 `codex/unified-generation-history`，HEAD `4af46b5fe49710468587ae7e3caa98490d135bdd`。因原分支包含基础剪辑/视频导出差异，本轮从 `origin/develop` 建立 `feature/generation-history-task-cards` 独立工作树，只 cherry-pick 生成历史相关的两笔现有改动；原工作树保持原状。未修改主仓 `D:\unicompapi-git`，未读取真实项目任务/作品数据，未调用真实服务商。

#### 最新交付记录（覆盖下方过程记录中的未完成状态）

- **最新负责人验收修订：精简卡片＋最近30任务（2026-09-24）**：覆盖下方历史“50个结束任务＋全部活动任务、加载更多、保留扩展分页范围”的产品规则。当前项目/模式按 task.createdAt 选择最新30个任务，成功、失败、生成中、待确认等共用30张额度；仍一任务一卡，不按Work数量拆卡或截断卡内作品。前端合并既有读模型的30个最新结束任务与活动任务后统一排序取30，无需改变IPC/持久化Schema；删除分页按钮、扩展页计数和分页边界状态，保留用户滚动接管与单飞刷新。限制只影响展示，更早任务及作品不删除。
- **最新卡片样式**：完成且只有一件作品时整卡为缩略图，去掉“最新执行：已完成”“作品1／1”；无作品时只保留一份状态图标/文字，去掉重复状态行与“暂无作品”；多作品才保留紧凑切换条；已有旧作品但最新执行未完成/失败时保留一个小状态标记。卡片外尺寸与时间线几何不变。
- **本次验证**：typecheck、lint、生产build通过（JS `index-BSEvorZu.js`）；22项应用Vitest、26项UI合同通过。完整生产Electron五模式宽窄布局、真实本地媒体、项目隔离、草稿删除、从105个混合任务中准确选择最新30个、生成六阶段30卡节点身份与几何一致均通过。60次原生滚轮事件至两帧p95约16.9ms、最大17.3ms，派发至观察绘制最大约103.8ms。共享组件同时验证单卡无多余文字、多作品切换、30条边界、刷新不抢滚动。未调用真实生成服务商、未删除历史数据、未提交推送或合并。
- **develop 同步与合并后验证**：2026-09-24 将 `origin/develop`（`87379db7b643316aa178215c314235c1ffffddec`）普通合并至本功能分支；`PLANS.md` 保留双方维护记录，动态参数合同测试同时保留通用候选清理与视频 `blockedReason` 专项断言。合并后的 typecheck、lint、build、完整 Vitest（1994/1994）、历史组件 Electron 和图片/视频提交收据 Electron 通过。完整 Node+UI 脚本为 384/385；唯一失败是本地缺少 V1.2.1 handoff 的 `SHA256SUMS.txt`，不得伪造或跳过。全应用 Electron 覆盖五模式、项目隔离和最近30任务检查，但在“生成期间滚动”阶段超时；同轮采样时 Electron 窗口已隐藏/最小化，scroll 两帧 P95 约 2033ms，故该性能读数无效，不能据此证明前台真实卡顿，也不能将该项记为通过。接管失败状态的 selection 回归修复了 `undefined` taskId 相等误匹配，定向 8/8 通过。最终推送提交号在本条补记。

以下条目为此前实现与复验过程；与以上最近30任务和卡片样式冲突的规则已由本次负责人决定替代。

- **负责人复验反馈后的补修（2026-09-24）**：新增原生滚轮 RED→GREEN 回归直接观察到生成期间滚动 400px 后，一次存储刷新把 scrollLeft 从14828拉回15228。修复用户滚动优先级，并将 React 被动 wheel 委托改为组件原生 `passive:false` 监听，横向消费时阻止外层页面同时滚动，边界仍允许正常滚动。
- **完成瞬间时间线收缩**：真实生产 main/preload/IPC＋磁盘监视复现100个结束任务＋5个活动任务在一个活动任务完成后从105张缩为104张（新结束任务占用分页名额，把最旧已加载任务挤掉）。现在按已加载最旧任务边界补读，必要时包含刚结束的更早活动任务；保留同一卡片节点、已加载范围及滚动锚点，离开返回仍保留范围。只保存 taskId/createdAt 作为视图边界，不缓存 Work/业务事实；代价是完成跨页或较早长任务时读取更多必要历史页，仍复用原有单飞读 owner。
- **新版动态复核**：最终生产构建 `index-CEQMAVAV.js`。定向28项 Vitest、26项UI合同、共享组件10组 Electron 检查通过；生产应用测试新增60次原生滚轮与真实存储六阶段状态写入，105张卡在观察到的中间DOM变化中数量/唯一taskId/节点身份/卡片高度/时间线位置均不变。原生事件到两帧 p95约16.7ms、最大17.2ms；主进程派发至观察到绘制（含测试轮询等待）最大约100ms。原脚本选择/输入/滚动两帧 p95分别约37.6/34.6/33.5ms，本轮无2秒停顿。历史2033ms样本的环境原因未获完全证明，不再把“窗口遮挡”当作确定根因；新增明确原生交互门槛 p95<100ms、最大<250ms，不能用纯脚本设scrollLeft或放宽门槛替代。
- **版本核对与入口纠正**：本轮开始实际运行进程来自 `D:\unicompapi-worktrees\unified-generation-history`，其生产 JS 为2026-09-23构建，源码仍有 `live-current` 匿名卡逻辑。这是已核实的版本事实，不能把该旧程序的表现当作新工作树验收。旧窗口结束后，现已启动 `D:\unicompapi-worktrees\generation-history-task-cards` 的正常应用（不再仅留样例窗口）；明确入口为 `outputs/start-fixed-app.cmd`。未覆盖/修改原工作树的无关编辑导出代码，未调用生成服务商，未提交推送或合并。
- **已实现**：一个 taskId 一张历史卡；固定一处预览，多个 Work 用“作品 n／总数”切换，最新执行状态独立显示；失败重试保留旧作品。创建、完成、取消、失败、结果待确认均保留任务身份。最新 execution 按 attempt 排序，旧执行晚到更新时间不覆盖新重试。
- **历史和选择**：项目＋模式＋媒体类型查询，不依赖当前或已经删除的草稿；按任务创建时间排序。首屏 50 个结束任务并保留全部活动任务，“加载更早任务”分页，加载前后保持可见滚动锚点。作品及状态手选不被其他任务更新夺走；离开返回保留作用域内所选 task/work、页数和滚动位置（有界 32 个视图状态，仅会话内，不缓存业务事实）。
- **安全关联**：只将同项目、同 Task、已关联 completed Execution 的 Work 放入卡中，要求 FileReference 同执行、available、校验通过且校验 Hash 与登记 Hash 一致。正式文件读取仍走既有媒体句柄门禁。
- **视频“处理中”**：主进程持久化 Task/Execution 接管状态后返回 taskId/executionId；原 VideoFeatureController.operations 继续拥有完整的有界轮询、接收和登记 Promise，waitForOperations 关闭/项目切换守卫仍等待它，不创建第二套后台循环。查询中断/轮询耗尽写入本地失败、retryability=unknown，历史投影为“待确认”，不声称远端失败或自动重新提交；接收异常保留明确阶段和允许的本地恢复条件。原有关闭/切换等待可能持续到该有界操作结束，未改为可强制终止的后台任务系统。
- **输入保护**：同一事件轮次双击只提交一次；回执后解除提交 busy；旧结果不清空等待期间新输入；离开页面后迟到响应不更新旧工作区。手选接管只在一轮新提交开始时重置，不在 preparing→requesting 中途再次重置。
- **窄窗口补修**：生产 Electron 800×720 复现专业生图切换 Work 时历史容器短暂从 813px 收缩到 282px，页面滚动范围随之缩短；最终尺寸恢复后滚动位置仍偏移。MutationObserver 记录证实是媒体句柄替换期间的自适应高度塌缩，单纯等待稳定或关闭局部滚动锚定不能解决。只在既有单列断点内为共享历史设定随窗口约束的高度，不改变滚动实现；五模式切换前后及中间 DOM 变更的历史高度、主页面 scrollTop 均保持一致。生产构建、相关布局合同 26/26、lint、平台审计和完整应用复测通过。
- **已观察 RED→GREEN**：单任务多预览块、首屏105条全量读取、执行次序/关联校验/取消任务缺失、持久化终态仍显示等待、异步查询和接收异常未落盘。修复后定向 Vitest **65/65**、UI/IPC/布局源码合同 **78/78** 通过；类型检查、全仓 lint、生产构建、平台审计（0违规）和 diff 检查通过。构建保留原有大 chunk 提示。
- **Electron 证据分层**：`verify-generation-history-app-electron.cjs` 使用真实生产 renderer/main/preload/IPC、隔离真实磁盘仓储和本地合成 PNG/MP4，验证五个模式、实际图片/视频预览、单预览、多作品切换、存储监视刷新、深色1280×900/浅色800×720布局、离开返回、实际临时图片草稿删除、第二项目隔离和100结束＋5活动任务分页。它采用预置任务事实，不声称真实服务商端到端生成。`verify-generation-history-cards-electron.cjs` 使用真实组件＋合成 storage，覆盖60件作品、键盘、手选状态、15次事件合并（峰值并发1，当前两页＋一次补刷共4次读取）和终态覆盖局部等待。`verify-generation-submission-receipt-electron.cjs` 使用真实提交工作区＋受控提交回执，覆盖双击、新输入与迟到回调。视频运行时专项测试使用真实运行时/仓储/状态机与受控供应商接受结果、查询和接收边界，覆盖完成、取消、查询异常、接收异常、轮询耗尽及原恢复路径。
- **性能测量边界**：i5-12400 / Windows x64 / Electron33.4.11，生产应用100结束＋5活动任务，选择/输入/滚动各30个样本。较早完整通过轮次中两次 requestAnimationFrame 的 p95 为33–35ms；最终验收窗口轮次分别为33.8ms、34.1ms、2033.6ms，三项p50均约32ms，测量期间无≥50ms长任务。存在明显的帧调度尾延迟波动，置顶也未完全消除，不能据此声称滚动性能稳定达标；其具体环境原因未完全确认，不据此改写业务代码。该指标是渲染机会，不是物理键盘/屏幕延迟，不声称相对旧版的百分比提升。完整报告和截图在 `outputs/generation-history-app/`。既有图生工作台108字符原生输入脚本最终通过14组检查，p95约17.2ms、边界调用0；首次曾观测到1次父重渲染，未改业务代码复跑为0，保留此时序波动事实，不保证任何环境绝对0重渲染。
- **工具边界**：Sliver 附带 stage checker 对既有 PLANS.md 返回 `MIGRATION_REQUIRED: stage schema is missing`；这属于项目原有文档格式与工具模板不匹配，不等于应用构建/测试失败。本次按负责人“最小改动、维护期”边界不迁移整份阶段文档，不声称该模板门禁或 Host 自动强制已验证。
- **验收入口与测量环境**：`outputs/start-history-acceptance.cmd` 自动复核后保留隔离样例窗口，操作说明见 `outputs/生成历史验收说明.md`。此前两帧等待约2033ms曾触发脚本120秒超时。测试脚本在采样期间保持窗口置顶，完成后恢复普通窗口；最终8组完整应用检查、10组宽窄布局及切换中间状态检查均通过，错误日志为空，验收窗口已打开。帧调度波动按上项单独记录，不与功能断言通过混同。
- **本轮生成中修复**：历史横向滚轮改为原生 `passive:false` 监听，避免 React 委托的被动 wheel 监听让外层 workspace 抢滚动；用户一旦滚轮、指针或键盘操作历史，后续生成刷新不再强制拉回最右端。补充了生成中原生滚动与多次执行状态变更的 Electron 回归，要求滚动事件可取消、卡片 taskId 不重复、同一卡片节点和几何保持不变。
- **交付位置**：`D:\unicompapi-worktrees\generation-history-task-cards`，分支 `feature/generation-history-task-cards`。此前两笔本地移入提交仍为 `77e4cd3`、`49ca210`；本轮后续改动未提交、未推送、未合并。原工作树和主仓保持原状；仅只读复用主仓开发用 FFmpeg 生成隔离样例，没有打包或登记二进制。
- **未验证/下一步**：真实服务商、真实用户项目、macOS、强制结束进程后的实际远端恢复不在本次隔离验收结论内。负责人验收五个入口的一任务一卡、作品切换、独立状态及新输入保护；通过后再另行决定提交/合并，不把人工验收提前记为通过。

以下审查与阶段记录保留用于追溯；其“未完成”以本节最新交付记录为准。

#### 用户结果与定义

- 历史的顶层单位是**持久化任务**：一个 `taskId` 在当前项目和创作模式下始终只有一张历史卡。卡片身份不能因状态从生成中变为接收中、失败或成功而改变。
- 卡片概括该任务最新一次 execution 的真实状态，并保留其全部已验证 Work 作为卡内可选择结果。一个任务重试失败但已有旧作品时，仍是一张卡，同时如实显示“最近一次执行失败”和既有作品；一个任务产出多个 Work 时也不删减作品。execution 明细继续由任务中心负责。
- 2026-09-24 负责人明确：外层一任务一卡，卡内只保留一个主要预览位置；多个作品通过“作品 1／2”及紧凑前后切换控件选择，最新任务状态单独显示。固定状态行、预览位、切换行，不堆叠缩略图；单作品不显示切换按钮。
- 未能关联到 `taskId` 的短暂提交进度只显示在当前预览/提交进度区，不得伪装成第二条历史记录。任务一旦从持久化读模型出现，就以该 `taskId` 更新唯一卡片。
- 历史汇总以任务数为单位；卡内作品数单独显示，避免把同一任务的多个 Work 算成多个成功任务。卡片顺序依据任务创建时间，状态更新不重排历史。
- 范围仍按 `projectId + workspaceMode + mediaKind` 隔离。清空输入、删除快速模式临时草稿、创建新草稿、离开后返回均不改变已持久化任务的历史归属。正式 Work 仍必须通过现有文件校验和登记链路。

#### 直接审查所得事实与边界

1. **截图支持存在两张不同状态卡，但不含身份字段。** 标记处分别是“生成中”和“正在接收”，按钮显示“处理中”；截图没有 Task/Execution/Work ID，不能单靠截图确认两张卡属于同一任务，也不能确认同一 Work 被重复登记。
2. **代码证实同一任务可被重复表示。** `GlobalReadModelController.listGenerationHistory` 对持久化任务生成最新 execution 状态项；`GenerationHistory.buildHistoryNodes` 又可能按页面局部 `submissionProgress` 添加无 `taskId` 的 `live-current`。例如持久化状态是 `receiving`、局部阶段仍为 `waiting` 时，当前按状态类别去重不会合并它们，会同时产生“正在接收”和“生成中”。这是截图现象的直接代码解释；仅缺少截图对应 ID 的运行证据，不能声称已还原那次真实任务。
3. **任务卡必须保留子记录。** Work 有 `sourceTaskId`，视频结果接收器会为同一 execution 下的每个已验证候选分别登记 Work；Task 也拥有多个 execution ID。因此按 `taskId` 去重整张历史项会丢有效作品/重试事实。正确模型是一个任务组卡片，状态和作品作为卡内内容。
4. **部分历史改动已存在，但验收不完整。** 当前读模型已支持按任务快照中的 workspace mode 查询，快速生图 runtime 也确实在提交后删除临时草稿；这使“按现存草稿 ID 查历史”的旧问题已有代码层修正方向。此次审查未找到覆盖“删除临时草稿后，经真实读模型仍能列出该 Work”的直接回归，不能把实现意图当成端到端证明。
5. **分页合同仍会漏活动任务。** 后端将状态项与 Work 混排分页；UI 仅请求 50 项且忽略 `nextCursor`。一个任务产生多个 Work 会挤占其他任务位置，活动任务可能被截断。任务分组后应按任务组分页，活动任务必须独立于已结束历史分页。
6. **当前已有刷新优化，但仍有刷新合并缺口。** UI 已从逐草稿查询改为一次项目/模式读；旧报告中的“20 个兄弟草稿导致 21 次 IPC”不能描述当前实现。另一方面，每个本地存储变化仍会立即递增刷新键，没有同作用域单飞请求和合并补刷机制；是否造成可感知卡顿需测量，不能用静态推断。
7. **长时间按钮“处理中”是相邻但独立的问题。** 视频 runtime 在 IPC 提交调用内执行有界远端轮询（默认最多 120 次，等待间隔上限 30 秒），提交面板要等该 Promise 返回才释放 busy。它能解释长等待，但不能仅据此诊断死锁。任务分组可修复重复历史卡，不能单独解决按钮长时间忙；不能只提前清 busy，必须先确认现有可恢复任务执行/接收机制如何接管后台工作。
8. **布局跳动是另一条有复现记录的问题。** 已有隔离 Electron 报告记录快速生图成功与空态的历史区坐标差 68.5 CSS px；当前分支已改布局样式，但本轮未跑完整 AppLayout 的前后对照。此项要作为独立 UI 验收，不把它误说成任务卡重复根因。

#### 唯一推荐实现方案

1. **在读模型按 taskId 组装数据。** 复用既有 storage IPC 与 `GlobalReadModelController`，读取范围为项目/模式/媒体类型；返回任务的稳定身份、创建时间、最新关联 execution 状态及其时间、按 `sourceTaskId` 关联的全部已验证 Work（每件保留独立 `workId` 和预览元数据）。不改数据库、不猜测旧任务模式、不把未校验远端结果当作品。只在确认现有调用方确有兼容需求时保留 draft 查询路径，不为假设中的外部消费者加兼容层。
2. **Renderer 只渲染任务卡。** `GenerationHistory` 以 `taskId` 为唯一顶层 key；卡内展示最新 execution 状态和作品计数，并允许逐件选择卡内 Work。移除以局部 phase 生成匿名历史节点的路径；该进度继续驱动当前预览和提交控件。相同状态的两个不同 taskId 必须是两张卡，同一个 taskId 的状态与 Work 必须同卡。
3. **用任务状态主导选择。** 点选卡片固定所选 `taskId`；选中任务有多个 Work 时保留逐件预览选择。其他任务完成/失败只更新自己的卡，不抢预览。选中任务没有 Work 时显示其当前状态；它已有 Work 且正在重试时继续允许预览旧 Work，同时清晰呈现最新 attempt 状态。按 task 创建时间排序，状态变化不改变顺序或用户滚动位置。
4. **按任务组分页并保留活动项。** 活动 task groups 单独返回，不受已结束历史游标挤出；已结束任务组用稳定 task cursor 加载更多。模式不明的旧任务不混入错误模式，由任务中心保留查看。UI 汇总统计任务数，Work 数作为组内结果数展示。
5. **性能优化以测量决定是否改。** 保留现有单次项目/模式查询，不恢复按草稿扇出。先测存储事件密集时是否出现同作用域重叠 IPC、旧响应覆盖或可感知卡顿；只有复现其中一项时，才在现有刷新 owner 加“单飞 + 一次合并补刷”。不为本次修卡预先增加媒体懒加载、轮询或通用缓存。
6. **维持固定布局合同。** 复用现有共享历史组件和当前右侧预览/历史结构，不加一级页面。对空态、活动状态、失败/待确认、单 Work、多 Work 逐态检查同尺寸窗口的预览、历史和操作区几何；卡内多作品不得撑动外部网格。

#### 分阶段执行与范围

本方案是 D2、T2 的跨读模型/IPC/UI 任务身份契约变更；布局通过可复现 Electron 门禁 T3 验证。当前已完成 task 卡合同、活动项独立返回、完成项分页和共享组件分组；完整隔离 Electron/main/preload/IPC 验收仍未执行。

1. **先写回归再改 owner。** 覆盖：同一 task 的 latest `receiving` + renderer `waiting` 只产生一张卡；两个不同 task 即使状态相同仍各有一张；一个 task 有已验证 Work 且最新重试失败时同卡保留两类事实；多个 Work 全部可选且不重复；模式/项目隔离及草稿删除后读模型仍可查。先观察直接回归失败，再实现。
2. **一次完成读模型和共享组件合同。** 改动边界原则上限于 `src/shared/storage-ipc.ts`、`src/platform/ipc/global-read-model-controller.ts`、`src/components/GenerationHistory.tsx` 及直接测试；只有编译/调用证据要求时才改 preload 或工作区消费者。不改 Task/Execution/Work 持久化 Schema，不迁移用户数据，不改模型选择、计费、授权或视频编辑器。
3. **补分页与选择回归，再量性能。** 使用 100 个历史任务和 5 个活动任务，证明活动项不丢、页游标无重复/漏项、乱序更新不改卡身份/顺序/选中预览。测量存储事件风暴下的重叠读取和 UI 延迟；只有当前实现造成重复 IPC 或响应覆盖时才加入单飞/合并补刷。现有“单查询代替逐草稿”的优化保留。
4. **走完整隔离 Electron 链路。** 使用真实生产 UI、main/preload/IPC 和真实任务/Work 存储，供应商边界注入本地确定性响应，覆盖同步/异步、慢响应、失败、待确认、重试、多个结果与乱序完成；不接触真实用户任务或付费服务。
5. **单独验收 busy 与布局。** 先验证视频长等待是否确由前台轮询造成，以及项目现有后台恢复机制能否承接轮询/接收；没有既有可靠 owner 时单独制定方案，不在任务卡修复中提前清 busy 或搭建第二套调度器。复测当前 CSS 在完整应用下的几何基线；只修仍能复现的跳动。
6. **隔离交付。** 因当前分支文件差异含无关基础剪辑/导出变更，若批准实现，先从最新 `origin/develop` 建立干净功能分支并仅移入本任务改动；保留原工作分支，不提交、不推送、不合并，直到负责人另行授权。

#### 验收标准

| 场景 | 必须满足 |
| --- | --- |
| 一个 task 同时有局部提交进度、最新 receiving 状态 | 只有以该 taskId 为身份的一张历史卡；局部预览可以显示进度，不生成匿名第二卡 |
| 两个 task 状态相同 | 两张独立卡，互不合并 |
| 一个 task 有历史 Work 和失败/待确认重试 | 一张卡中保留 Work 与最新执行状态；不删除、不误报成功 |
| 一个 task 有多个已验证 Work | 一张顶层卡，所有 Work 可逐个预览；每个 workId 只出现一次 |
| 手选任务期间其他任务更新 | 所选 task/work 不变；卡片身份与提交排序不变 |
| 临时草稿删除、清空输入、离开返回 | 按项目和模式从持久化 task 恢复；不依赖当前 draftId 或组件仍挂载 |
| 超过 50 个任务且含 5 个活动任务 | 活动任务不被结束历史分页挤出；连续翻页无缺项/重复项 |
| 状态/内容变化及多作品卡 | 同一窗口/任务栏条件下，外部预览、历史和操作区几何不跳动；内容不溢出 |
| 存储事件密集到达 | 先记录同作用域并发 IPC、过期响应和交互延迟；无复现时不增加刷新调度逻辑，有复现时验证单飞与一次补刷 |
| 100 任务/5 活动任务下交互 | 记录设备、构建、样本及输入/选择/滚动 p50/p95、长任务和 IPC 次数；不以未经基线证明的静态数字宣称流畅 |
| 完整验收 | 定向 T2 回归、完整隔离 Electron T3、typecheck/lint/build、相关平台审计和 diff 检查通过；真实服务商/真实用户数据/macOS 未测项明确列出 |

#### 当前完成与剩余

- 2026-09-23 实施结论经 09-24 复核修正：当时仅完成部分实现，不能视为方案已完成；前端仍有 50 条截断、Work 计数、按状态时间重排及多预览块堆叠遗漏。
- 本轮新增：DTO 以 task 为单位携带 latest 状态和全部 verified works；活动 task 独立返回，完成历史以 task 卡游标分页；页面按 taskId 聚合状态/Work，一件任务只产生一个顶层节点，并逐件保留作品预览。局部提交进度继续只在预览区显示，不补匿名历史节点。
- 验证通过：`pnpm typecheck`、`pnpm lint`、`pnpm build`、读模型性能测试 5/5、历史状态应用测试 10/10、历史 UI 合同 7/7、`git diff --check`。读模型合成场景覆盖 100 task、单任务多 Work、活动 task 独立于已结束分页；性能基线 10 项目/1000 实体 cold history 约 3 ms。生产构建只有既有大 chunk 提示。
- 未完成：真实 Electron/AppLayout/main/preload/IPC 交互验收；截图任务 ID 对照；存储事件风暴与刷新竞争测量；已删除临时草稿的真实读模型专项测试；视频长时间 busy 的后台接管审计；当前 CSS 完整页面几何复测；实际项目人工验收。此前隐藏 Electron 探针只渲染共享组件并用合成 IPC，不能代替这些验收。
- 不调用已耗尽预算的 Vidu，不读取凭证或真实用户数据。
- 当前 worktree 为 `D:\unicompapi-worktrees\generation-history-task-cards`，分支 `feature/generation-history-task-cards`；此前移入两笔改动实际创建了本地 cherry-pick 提交 `77e4cd3`、`49ca210`，不能把分支整体描述成“没有新提交”。后续任务卡修改仍未提交，未执行推送或合并。

#### 2026-09-24 单预览卡修正及验证

- 本轮范围为共享历史组件、局部 CSS、直接测试与本记录；未新增 IPC/数据库变更、服务商调用或 Git 提交。五个生成工作区继续复用现有共享组件。
- 每任务固定一处预览，状态单独显示，卡内前后按钮切换作品并保留 workId 选择；组件按任务身份保留局部选择，刷新与新增结果不抢走手选作品。选中任务状态后刷新不再强制跳回旧 Work，任务 ID 直接从节点读取，不解析 ID 字符串后缀。
- 配套修正：汇总输入改为每任务一次，排序使用 Task.createdAt；去掉加载结果后的 Work/task 50 条截断，确保卡内第 51 件以后结果可达。当前仍会遍历所有历史页；用户触发加载更多与存储事件风暴下刷新成本优化尚未完成，不能称分页性能已收口。
- 测试先行：`scripts/verify-generation-history-cards-electron.cjs` 在修改前准确失败，三张卡预览块数为 `[1,3,1]`；修正后 `[1,1,1]`。真实 React/生产 CSS、合成 storage 边界、阻断 HTTP(S)、隔离 profile，验证深色 1200×900 与浅色 800×720 下切换、最新失败仍可看旧作品、追加结果、状态手选与刷新、Enter 键及焦点、各态几何不变和控件包含关系。60 件作品逐件切换至第 60 件成功，始终一处预览。报告与截图：`outputs/generation-history-cards/report.json`、`dark-1200.png`、`light-800.png`。
- 定向验证：类型检查、应用/读模型 Vitest 25/25、五个相关 UI Node 文件 46/46、全仓 lint、生产构建、平台审计（0 违规）、diff 检查通过；构建保留既有大 chunk 警告。该测试不经过真实 main/preload/存储链路，不作为整应用 E2E、视频播放或 macOS 验收；视频 busy、草稿删除链路、事件风暴、全应用几何与用户项目验收仍未完成。技能 Host 级自动门禁未验证。
