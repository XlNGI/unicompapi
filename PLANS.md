...
当前状态：阶段 5｜图片创作进行中。阶段 4 已正式收口；阶段 5 B1、A1、B2、B3 已合并到 `develop`，A2 快速生图页面已在 `feature/quick-image-ui` 完成并等待负责人验收。
...
下一步在本分支通过负责人手工验收后提交合并，再从最新 `develop` 单独创建 `feature/professional-image-ui`；专业生图、图片识别、图片编辑和图片转提示词不混入当前分支。
