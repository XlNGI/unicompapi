...
功能分支新鲜验证：相关 Vitest 88/88、UI/媒体合同 54/54、Electron 时间线恢复 20/20、过期恢复 17/17、完整预览 19/19、typecheck、目标 ESLint、生产构建通过。构建保留既有 chunk 大小提示。正在进行真实 Windows Electron 最小化超过 20 分钟的隔离验收，使用真实时间、5 分钟本地媒体有效期和生产 Range 响应逻辑；素材与 IPC 仍为隔离夹具，不写用户项目，睡眠/锁屏不在本轮自动执行范围。结果到达后更新本记录，再进行最终合入。 首次真实等待结束后测试窗口已被关闭，恢复检查未执行；保留失败报告，不记为通过。重新验收增加关闭事件记录，并将测试窗口最小化后隐藏，避免任务栏中的测试窗口被误关。


最终复验：真实 Windows Electron 后台最小化并隐藏 1,210,009ms（20 分 10 秒），未改时钟；恢复窗口后静态缩略图与封面地址保持不变，时间线和导出成品均换取有效地址并从原位置续播，5/5 通过。证据 outputs/editing-real-idle/after/report.json、before-idle.png、after-idle.png；首次窗口关闭的失败报告单独保留，未当作通过。实际协议采用 5 分钟有效期及生产 Range 响应实现；仍是合成素材/隔离 IPC，不等同全部用户项目、睡眠/锁屏或 macOS 验收。

已将 develop 8e7d331 普通合并进功能分支（91a245f）；App.tsx 保留新版 ChatModelSelection 和剪辑 selectedTaskId，PLANS 保留双方记录。合并候选 Vitest 95/95、UI/媒体合同 57/57、时间线恢复 20/20、静态图片 5/5、导出恢复 10/10，以及 typecheck、目标 ESLint、生产构建通过。拖动实际采集 74 帧，最大绘制间隔约 198ms，松手定位约 77ms。候选与实际合并后的产品代码逐文件一致，当前工作目录构建也通过。上述五项旧失败和此前拖动空采样已关闭；未运行全应用全量测试。

PR：https://github.com/XlNGI/unicompapi/pull/9 。负责人已授权在这些验证通过后使用 merge commit 合入 develop，保留功能分支；最终远端合并 SHA 和分支核对结果记录于本地 outputs/merge-develop-check/合并执行报告.md，GitHub PR 状态为实际合入事实。
