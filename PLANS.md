...
当前状态：阶段 5｜图片创作已于 2026-07-23 完成联调并正式收口。真实在线图片操作继续等待后台适配器契约；阶段 6 启动前必须先形成并批准视频创作任务拆分。
...
项目负责人在开发者 A/B 最新 `develop`（`1a711b8`）完成统一联调。五个图片模式、项目内草稿、受控单图输入与预览端口、动态能力预检、确认快照、任务/执行/调用分层、结果接收边界和真实离线页面已经接通。
...
Windows 原生文件选择器已确认由页面打开；受当前桌面自动化焦点限制，本轮未把“选择文件后预览”作为人工点击证据，单图内容签名、尺寸、字节、Hash、登记、短期预览和异常路径继续由 5 项本地媒体平台测试覆盖。该限制不影响阶段 5 本地/离线能力收口，也不等同于真实在线图片能力完成。
