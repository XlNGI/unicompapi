...
当前状态：阶段 7 已完成联调并正式收口；阶段 8 规划已批准，B1 已合并 `develop`。A1 已在 `feature/local-settings-shell` 完成设置页壳、固定 10 分类、常规设置、自动保存和恢复默认影响确认，当前等待项目负责人 Windows Electron 手工验收，验收前不上传、不合并。A2/A3/A4 继续等待 B2/B3/B4。生产更新源、生产媒体组件分发、硬件能力与 macOS 实机证据仍未具备，不得伪造为可用。规划见 `docs/active/阶段8-技术架构.md` 与 `docs/active/阶段8-任务拆分.md`。
...
下一步：开发者 B 可启动 B2 存储、任务性能和本地媒体端口；A1 等待项目负责人 Windows Electron 手工验收。工程记录见 `docs/active/阶段8-B1-设置契约与仓储记录.md`。
...
A1 门禁为 121 项 Node UI/IPC/工具链契约测试与 205 项 Vitest 领域/平台测试，共 326 项通过、0 失败；5 项真实 FFmpeg 测试因隔离工作树没有本地 `.tools` 跳过。TypeScript、ESLint、生产构建和差异检查通过。浏览器在 1920×1080、1280×900、1080×800 下无横向溢出，自动保存、主题应用、分类搜索、未接能力和恢复确认通过，控制台无警告或错误；Electron 生产构建启动烟测 4 个进程均响应并完成清理。工程记录：
