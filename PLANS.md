...
当前状态：阶段 7 已完成联调并正式收口；阶段 8 规划已批准，B1 已合并 `develop@1eb3dd8`，A1 已通过 `b1bd313` 合并 `develop`。B2 已在 `feature/local-settings-storage-performance-media` 完成并同步 A1 最新基线，等待集成门禁后合并。B2 建立受控目录登记、路径隔离 DTO、目录迁移与故障恢复、清理白名单、动态性能策略快照、真实 FFmpeg 状态和两阶段高风险执行。A2 必须等待 A1 与 B2 同时进入最新 `develop`；A3/A4 继续等待 B3/B4。生产更新源、生产媒体组件分发、硬件能力与 macOS 实机证据仍未具备，不得伪造为可用。规划见 `docs/active/阶段8-技术架构.md` 与 `docs/active/阶段8-任务拆分.md`，B1/A1/B2 工程记录均保留在 `docs/active/`。
