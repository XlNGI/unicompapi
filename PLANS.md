...
当前状态：阶段 7 已完成联调并正式收口；阶段 8 规划已批准，B1、A1、B2 已合并 `develop`。B3 已在 `feature/local-settings-platform-controls` 完成最小授权、代理隔离测试与事务回滚、独立 `safeStorage` 凭证、通知降级、版本化快捷键、跨平台绑定隔离和恢复默认事务；365 项测试、TypeScript、ESLint、生产构建、差异检查与 Windows Electron 烟测通过，等待合并。A2 可从当前 `develop` 启动；A3 必须等待 B3 合并，A4 继续等待 B4。生产更新源、生产媒体组件分发、硬件能力与 macOS 实机证据仍未具备，不得伪造为可用。规划见 `docs/active/阶段8-技术架构.md` 与 `docs/active/阶段8-任务拆分.md`，B1/A1/B2/B3 工程记录均保留在 `docs/active/`。
...
未完成项：A2 的存储/性能/媒体面板尚未接入；B3/B4 的隐私、代理、通知、快捷键、诊断和更新不在本分支；硬件编码、生产 FFmpeg 分发、签名、编解码器白名单、专利/商业审查和 macOS 实机证据继续等待阶段 9/10。B2 已合并 `develop`，A2 可从同时包含 A1 与 B2 的最新基线启动。工程记录见 `docs/active/阶段8-B2-存储性能与媒体能力记录.md`。
...
验证结果：121 项 Node UI/IPC/工具链测试与 244 项 Vitest 领域/平台测试，共 365 项通过、0 失败、0 跳过；TypeScript、ESLint、生产构建和差异检查通过；Windows Electron 生产构建隐藏启动 4 个进程全部响应，清理后残留 0。工程记录见 `docs/active/阶段8-B3-隐私代理通知与快捷键记录.md`。

未完成项：A3 四个用户控制面板尚未接入；B4 诊断/导入执行/重置/更新仍未实现；macOS 实机完整证据留到阶段 9。B3 分支等待合并，A3 必须从合并 B3 后的最新 `develop` 启动。
