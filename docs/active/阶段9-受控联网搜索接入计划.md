...
日期：2026-09-04  
状态：`in_progress`（W1/W3/W4 基础合同已实现；W0/W2/W5/W6 未完成）
文档类型：工程侧实施计划
...
2026-09-04 已从同步后的 `develop` 创建 `feature/web-research-foundation`，落地以下不发起真实联网的基础合同：

- `src/shared/web-research-ipc.ts`：预览、授权、取消、状态 DTO 与严格请求解析；
- `src/application/conversation-web-research-service.ts`：本地 BM25 优先、授权绑定、planHash/revision 校验、取消/过期和来源统一投影；
- `src/platform/search/credentialed-web-search-transport.ts`：固定 HTTPS origin、主进程凭证回调端口和受控 transport 错误映射；
- `src/platform/ipc/conversation-web-research-controller.ts`、Electron IPC/preload 和 `ChatPage.tsx`：对话内外发预览与明确授权入口；
- 自动化覆盖 Application 3/3、联网合同 5/5、受控 transport 4/4、IPC 合同 2/2。

默认运行时使用 `UnconfiguredWebSearchTransport`。因此 W0 服务商决策、W2 官方适配器、W5 持久缓存/审计治理和 W6 Windows 真实请求仍未完成；当前不会读取真实凭证、发送 HTTP 或产生费用，也不能宣称联网搜索已可用。
