...
状态：流程 1—2 已合并，流程 3 已完成分支实现；流程 4—7 与真实联网、收费验证尚未完成
...
流程 1 已通过 `c03693f`、流程 2 已通过 `7015624` 非快进合并 `develop`。流程 3 已在 `feature/vidu-runtime` 完成分支实现与门禁，新增唯一 Vidu 服务商包、凭证回调边界、受控 HTTP transport 契约、端点/代理/超时/取消/响应上限、错误脱敏和合成 `/ent/v2/credits` 验证端口；实现与验证事实见对应流程工程记录。流程 1—7 的连续实施授权不包含真实 Token、真实 Vidu 联网或收费调用。
