...
状态：流程 1—3 已合并，流程 4 已完成分支实现；流程 5—7 与真实联网、收费验证尚未完成
...
流程 1 已通过 `c03693f`、流程 2 已通过 `7015624`、流程 3 已通过 `3a09d29` 非快进合并 `develop`。流程 4 已在 `feature/vidu-image-adapters` 完成分支实现与门禁，新增两个同步图片协议适配器、受控单图复核、20MB 请求门禁、同步 receipt 结果下载/解码、图片探测、SHA-256、原子落盘与 Work 幂等恢复；Image V1 的鉴权和 `images` 结构仍保持未验证并在生产绑定层阻断。实现与验证事实见对应流程工程记录。流程 1—7 的连续实施授权不包含真实 Token、真实 Vidu 联网或收费调用。
