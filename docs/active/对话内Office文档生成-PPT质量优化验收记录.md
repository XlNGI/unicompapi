...
## 15. 2026-09-04 PPT 总页数修订失败修复

### 15.1 人工问题与根因

Windows Electron 实际会话中，上一版“关于龙的PPT”成功后，用户依次输入“内容太少了加到5页”和“修改文档”。Provider 返回了包含 5 个 section 的 PPT JSON，其中第 2 至第 5 个 bullets `items` 数组各缺少一个闭合方括号；同一响应的有限本地恢复可以恢复全部 5 个 section，因此最终 `invalid_outline` 不是恢复器失败。

真正阻断位于 Application 修订编排：代码只读取当前 assistant 之前最近 user message 的 `displayContent`，而两条工作流 user message 都只有 `content`，导致修订请求被判定为缺失。即使仅回退 `content`，只取最后一句也会得到“修改文档”，仍丢失前一句“加到5页”。同时，既有局部 patch Agent 只允许明确章节/页码目标，不能安全表达全篇页数调整；提示词也没有明确说明系统会另行生成封面与结束页，模型因此把“5页”误解成 5 个正文 section。

### 15.2 受控修复

- Application 按原顺序聚合当前 assistant 前连续 user 消息，优先 `displayContent`、否则使用 `content`；对历史链路保留最近 user 消息兼容回退。截图中的两句现在形成完整请求“内容太少了加到5页\n修改文档”。
- 新增确定性 PPT 总页数解析，只接受“加到/扩展至/总页数/5页PPT”等明确总数表达；“修改第5页”和“增加5页内容”不被误判为总页数修订。
- 明确总页数修订由 Application 单独授权为全篇结构替换，不进入局部 patch Agent。支持范围沿用统一 3 至 40 页门禁；系统固定生成封面与结束页，因此 5 页要求必须恰好返回 3 个正文 section，并拒绝模型自带 cover/closing section。标题由 Application 保留上一版值。
- 提示词明确总页数包含 1 页封面和 1 页结束页，5 页必须返回 3 个正文 section；每个 section 限制为单页容量，避免自动续页。Runner 在临时 PPTX 的 OOXML、内容与 Hash 校验阶段读取真实 slide 数，只有恰好等于用户要求时才原子发布并登记 Work；不匹配时返回独立 `page_count_mismatch` 安全错误并保持零 Work。
- 将依赖本机项目绝对路径的临时诊断测试删除，替换为自包含的五 section 重复缺括号回归夹具；仓库不保存用户项目路径或原始完整会话文件。

### 15.3 验证结果

- 定向 Application、提示词、恢复器、Runner 与 fail-closed 回归最终 87/87；聊天文档/透明度 UI 合同 19/19。
- 完整 `pnpm test` 最终为 Node/UI 合同 353/353、Vitest 183 个文件 1139/1139，合计 1492 项通过，0 失败、0 跳过。首轮曾因新提示含合同禁止的“重新生成”出现 1 项失败，修正文案后通过；第二轮曾因请求聚合过窄触发既有修订 fail-closed 5 项失败，加入历史兼容回退后全部通过，未放宽安全断言。
- `pnpm typecheck`、`pnpm lint`、`pnpm build` 全部通过；生产构建转换 2069 个模块，仅保留既有 Vite CJS 与大 chunk 警告。
- 平台审计扫描 415 个文件零违规；交接校验 50 项、27 份资产零失败；恢复审计与阶段 9 关闭门禁均为 `passed`；`git diff --check` 通过。

### 15.4 未验证边界与人工复验

本轮未调用真实 Provider、未读取或验证凭证、未产生收费请求，也没有自动重试或修改既有失败会话事实。自动化已经真实生成并解包核对 5 页 PPTX，但 Windows Electron 中完整的“生成上一版 → 输入内容太少了加到5页 → 修改文档 → 打开新版”仍需在加载新主进程与 Renderer 代码后人工复验；必须确认新版卡片出现、PowerPoint/WPS 实际页数为 5、旧版文件保留、作品库中新 Work 带父版本关系。若真实模型仍返回 5 个正文 section 或内容造成续页，系统应显示明确页数不匹配并保持零新 Work，不能标记成功。

## 16. 2026-09-07 物理页目标与确认范围跟进

### 16.1 根因收紧

上一轮修复已将“第 N 页/张”与“第 N 章/节/部分”分开，但平台执行器仍可能把带有相同章节标题的续页一并纳入物理页目标；同时旧版 PPT 页脚没有稳定对象名时，结构解析和清空逻辑无法可靠区分页码与正文数字。本跟进将物理页目标定义为单一物理 slide，章节目标继续保留连续续页语义。

### 16.2 实际修改

- `targetUnit: page` 在 Office 执行器和 Runner 范围校验中只授权 `pageNumber` 指向的 slide，并要求页标题与受控目标一致；`targetUnit: section` 和无 `targetUnit` 的历史章节 patch 仍可扩展连续续页。
- PPT 生成器为页脚数字写入固定 `UniComp Page Number` 对象名；结构解析优先按对象名和 slide 序号排除页码，旧文件仅在显式物理页 patch 时使用最后数字文本回退，避免普通数字正文在常规读取中被误删。
- 破坏性物理页清空保留标题和页脚页码，并在标题不匹配时 fail-closed；确认卡补充文件名、物理页码、操作和原文件/其他页面保留说明。
- Application 在父 Work、受控序号目标、明确清空语义和本地 Revision Agent 同时存在时，先读取并校验父版本大纲，再直接生成受控 `clear_section` 补丁；本轮模型响应不再作为该清空操作的大纲输入。范围不明确的清空、普通语义改写、扩页和新建文档仍必须经过原有模型大纲编译与校验。

### 16.3 验证结果

- 定向 Application、Platform、Runner 回归 103/103；Node/UI 合同 353/353；完整 Vitest 183 个文件 1143/1143，0 失败、0 跳过。新增服务级回归以无效模型文本重放“将第二页的内容清空”，断言本轮文本不进入大纲编译器、父版本大纲仍进入本地 Agent，且输出补丁只授权物理第 2 张。
- `pnpm typecheck`、`pnpm lint`、`pnpm build`、`git diff --check`、平台审计、交接校验、恢复审计和阶段 9 关闭门禁全部通过。生产构建转换 2069 个模块，仅保留既有 Vite CJS 弃用和大 chunk 警告。

### 16.4 未验证边界与人工复验

本跟进没有调用真实 Provider、联网、读取凭证或启动 Office GUI。当前 Renderer 仍会先发起一次内容响应，但确定性清空的 Application 执行已不再依赖该响应的大纲格式；完全免 Provider 的本地快速路径尚未实现，不在本轮自动化结果中宣称完成。仍需在最新 Windows Electron 构建中人工验证：对含封面、正文续页和页脚数字的真实 PPT 执行“清空物理第 2 张”，确认即使模型响应不是文档 JSON 也只改目标 slide；执行“清空第二章”时确认连续续页仍按章节范围处理；确认卡显示文件和保留范围；新版文件可在 PowerPoint/WPS 打开，旧版 Work 与父版本关系保持不变。macOS 实机目标仍为 `required=false`、`deferred`。

## 17. 2026-09-07 连续多轮修改状态一致性修复

### 17.1 真实根因

实际会话中，第一轮“将第二页的内容清空”生成的新 PPTX 已正确清空物理第 2 张，但 assistant 消息仍保存模型在本轮提出、且与实际补丁结果不一致的大纲。第二轮“将第三页的内容清空”继续把这份模型原始大纲当作父版本，导致本地 Revision Agent 看到错误状态并判断没有产生变化，最终以 `unvalidated_output` 安全失败。Provider 本轮正常完成，磁盘、网络和 PowerPoint 均不是根因。

根因不是“第一次”和“第二次”需要不同分支，而是系统同时存在“模型建议状态”和“实际登记 Work 状态”两个竞争真源。修改轮次越多，偏差越容易累积。

### 17.2 受控修复

- `DocumentMessageResult` 新增可选 `validatedContent`，保存实际传给 Runner、通过 Application 范围校验并生成已登记 Work 的最终大纲；字段限制为非空且不超过 1,000,000 字符，旧消息没有该字段时仍可读取。
- 每轮修改统一以父 Work 对应消息的 `validatedContent` 为基线；仅对没有规范化状态的历史消息回退到原 assistant 内容。成功生成后把本轮最终大纲随新 Work 一起挂回消息，因此第三轮及以后重复同一规则，不存在首轮/次轮特判。
- Renderer 组织下一轮模型输入时同样优先使用 `validatedContent`。模型原始返回继续保留为审计和诊断资料，但不再覆盖实际文件状态。
- IPC 保留 `revision_scope_violation`、`revision_patch_failed`、`unvalidated_output` 三类具体错误码。实时失败和重启后恢复的失败状态均显示对应中文建议，并明确原文件未改变，不再统一退化成“文档生成失败”。
- 版本语义不变：每次成功修改创建新 Work 并登记 `parentWorkId`，父 Work 和原文件保持不变；只有本地文件、内容、布局、Hash、发布和登记门禁全部通过后才更新下一轮基线。

### 17.3 验证结果

- 新增连续两轮回归：第一轮模型大纲故意清空错误页面，本地补丁实际清空物理第 2 张；第二轮断言 Revision Agent 收到的是第一轮已验证大纲而非模型原文，并最终得到物理第 2、3 张均按顺序清空的规范化状态。
- 领域测试覆盖 `validatedContent` 解析、消息持久化、旧数据兼容、空值和超长拒绝；平台测试覆盖 JSON 仓储重新读取后规范化大纲仍存在，以及三类修改错误码完整穿过 IPC。
- 定向 Vitest 3 个文件 51/51，聊天 UI 合同 19/19，新增仓储往返后 controller 单文件 20/20；`pnpm typecheck`、`pnpm lint` 均退出码 0。
- `pnpm build` 退出码 0，Vite 转换 2069 个模块；仅有既有 Vite CJS 弃用和大 chunk 警告。
- 完整 `pnpm test` 退出码 0；当前清单为 Node/UI 353 项、Vitest 1153 项，共 1506 项。
- 阶段 9 关闭门禁、恢复审计、415 文件平台审计、50 项/27 资产交接校验和 `git diff --check` 均通过；恢复审计为 0 安全违规、0 禁止产物。

### 17.4 产品保证与未验证边界

对系统已支持且能通过结构、范围和文件校验的 Word、Excel、PPT 修改，连续任意轮次都使用最新已验证 Work 状态，不依赖“第几次修改”。这不等于承诺任意自然语言指令永不失败：目标含糊、范围越界、当前 Recipe 不支持、内容无实质变化、用户取消或外部环境故障仍会安全停止，但会给出可操作原因且保留原 Work。

本轮没有调用真实 Provider、联网、读取凭证、产生费用或启动 Office GUI，也没有修改既有失败会话事实。仍需在加载最新代码的 Windows Electron 中按真实路径复验“清空第 2 张 → 基于新版清空第 3 张 → 打开最终文件”，核对两轮新版卡片、最终 PPTX 内容、旧文件保留和父 Work 链；macOS 仍为延期目标。

## 18. 2026-09-07 明确清空操作免 Provider 路径

### 18.1 截图问题与业务根因

实际会话在已有“关于龙的 PPT”后输入“将第二章的内容清空”，模型候选 `deepseek-v4-pro` 返回 `newapi.authentication_failed`，页面显示“AI 内容生成未完成，文档未生成”。失败码来自 NewAPI 运行时的真实鉴权错误映射，现有 UI 对错误性质的说明正确；业务问题是本地可以确定的清空操作仍被强制置于模型响应之后，因此凭证、模型权限或路由故障会在本地修订开始前中断整个操作。

### 18.2 受控实现

- 新增 `prepareDeterministicRevision` IPC，只接收会话 revision、workflow revision、文档类型和父 Work ID，不接收文件路径、凭证、服务商或由模型决定的工具参数。
- Application 回读并交叉校验已确认 workflow、原始用户消息、结构化页/章目标、目标文档消息、父 Work 和 `validatedContent`。只有单一页/章且整句明确表达清空/删除内容、无附件和外部/内部资料检索时才进入本地路径。
- 命令语法明确拒绝“不要清空”、多目标、“清空并改写”、新增/扩写/替换等混合操作；计划目标与原句序号或单位不一致、目标越界、父版本缺少规范化状态时在创建文件前安全失败。
- 本地路径创建受控 assistant 消息并持久化 `validating_outline` 状态，随后复用既有本地 Revision Agent 和文档生成服务。取消、重启中断恢复、范围校验、真实 Office 结构比对、渲染/布局校验、Hash、原子发布、Work 登记、`parentWorkId` 和原文件保留语义不变。
- Renderer 在该路径中不检查模型可用性，也不调用 `chat.startResponse`；普通改写、新建、扩页、歧义请求和需资料的请求仍使用原模型链路。因此 NewAPI 鉴权故障不会再连带阻断受支持的单目标清空，但不会被误报为 NewAPI 凭证本身已经修复。

### 18.3 验证结果

- Application 回归真实构造“父 Work + 已确认 workflow + 将第二章的内容清空”，不提供任何 Provider 输出，验证本地完成消息、`clear_section` 补丁、新 Work、父版本保留和 workflow `completed` 闭环。
- 反向回归覆盖多目标、清空并改写、普通改写、否定清空、额外尾随操作与未声明 IPC 字段，均未进入本地执行；既有越界和范围违规门禁继续通过。
- 定向 Application/Agent/Controller 与真实 JSON 会话仓储为 65/65，聊天 UI 合同 20/20；仓储回归确认本地 completed assistant 消息只推进一个 conversation revision，且可由新仓储实例重新读回。完整 `pnpm test` 为 Node/UI 354/354、Vitest 183 文件 1158/1158，合计 1512 项通过，0 失败、0 跳过。
- `pnpm typecheck`、`pnpm lint`、`pnpm build`、平台审计（415 文件零违规）、交接校验（50 项、27 资产零失败）、恢复审计、阶段 9 关闭门禁和 `git diff --check` 均通过；生产构建转换 2069 个模块，仅保留既有 Vite CJS 弃用和大 chunk 警告。

### 18.4 未验证边界

本轮没有调用真实 NewAPI 或其他 Provider，没有读取、修改或验证任何凭证，也没有产生费用。`newapi.authentication_failed` 对应的凭证、模型权限或路由配置仍需独立排查，复杂改写仍会受其影响。本轮尚未启动 Windows Electron/PowerPoint GUI 人工复验；需完整退出并重启 Electron 后，在最新代码上复测“将第二章的内容清空”，确认不出现 Provider 执行记录、生成新版卡片、第二章正文清空、其他页面与旧文件保留、作品库父版本关系正确。macOS 仍为 `required=false/deferred`。
