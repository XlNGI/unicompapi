...
状态：PR0—PR6 已实施并完成 Windows x64 收口；macOS 延期，阶段 10 未启动。
...
PR0—PR4 已依次完成性能基线、项目批量快照、单次任务时间线与调用分页前置、共享任务 Store/消费汇总缓存，以及生成历史分页/按需媒体。PR5 在 Windows x64 健康 NVMe SSD 的 10 项目、1,000 Task 合成规模下确认冷任务列表 19.49 ms、热读 median 0.62 ms/P95 0.91 ms、单草稿历史冷读 2.92 ms，已明显达到目标，因此按条件跳过磁盘索引、缩略图缓存和数据库。

PR6 最终复测为冷任务列表 18.62 ms、热读 median 0.69 ms/P95 1.21 ms、单草稿历史冷读 3.49 ms；调用分页也已固定为轻量候选过滤、排序和分页后只构建当前页完整 DTO。完整 Node/UI、Vitest、TypeScript、ESLint、生产构建及工程审计门禁通过，Windows 生产 Electron 4/4 响应、1 个可见窗口、stderr 0、正常关闭后残留 0。详细证据见 `docs/active/生成历史与任务中心性能优化-PR0-基线记录.md` 至 `docs/active/生成历史与任务中心性能优化-PR6-Windows收口记录.md`。

项目 JSON 始终是唯一权威事实源；本专项没有调用真实服务商、读取真实凭证或产生收费请求，没有提交 `.tools/`/FFmpeg，也没有执行 macOS 实机或启动阶段 10。
